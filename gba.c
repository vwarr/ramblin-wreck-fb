...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  for (int i = 0; i < height; i++) {
    volatile u16 *dst = &videoBuffer[OFFSET(row + i, col, WIDTH)];
    DMA[3].src = &color;
    DMA[3].dst = dst;
    DMA[3].cnt = width | DMA_16 | DMA_ON | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
  }
}
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[3].cnt = DMA_16 | DMA_ON | DMA_NOW | WIDTH * HEIGHT | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  // DMA[3].cnt = WIDTH * HEIGHT
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int i = 0; i < height; i++) {
    ...
    DMA[3].cnt = width | DMA_16 | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  }
}
...
void fillScreenDMA(volatile u16 color) {
  DMA[3].src = &color;
  DMA[3].dst = videoBuffer;
  DMA[3].cnt = DMA_16 | DMA_ON | DMA_NOW | WIDTH * HEIGHT | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
}
